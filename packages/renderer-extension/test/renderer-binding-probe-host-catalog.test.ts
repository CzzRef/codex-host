...
vi.mock("../src/renderer-draft-worktree-picker.js", () => ({
  installRendererDraftWorktreePicker: () => ({ refresh: vi.fn(), dispose: vi.fn() }),
}));

vi.mock("../src/renderer-composer-prompt-reuse.js", () => ({
  installRendererComposerPromptReuse: () => ({ refresh: vi.fn(), dispose: vi.fn() }),
}));

vi.mock("../src/renderer-turn-header.js", () => ({
  installRendererTurnHeader: () => ({ refresh: vi.fn(), dispose: vi.fn() }),
}));
