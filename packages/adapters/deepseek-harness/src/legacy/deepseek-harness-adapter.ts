...
class DeepSeekHarnessSession implements HarnessSession, DeepSeekHostSubscriber {
  ...
  > {
    ...
    const active = createActiveTurn(command as TurnStartCommand);
    this.#active = active;
    try {
      unwrapRpc(
  ...
  /**
   * The Legacy generation injects a steer with `mode: "steer"`, keeping the
   * Host Turn open. Attachments degrade to their path line because this
   * prompt content is text only.
   */
  async #steer(command: TurnSteerCommand): Promise<HarnessResult<TurnSteerAccepted>> {
    const active = this.#active;
    if (!active || active.command.turnId !== command.turnId) {
      return { ok: false, error: invalidState("DeepSeek Harness steer requires the active Turn") };
    }
    const text = hostInputPromptText(command.input);
    if (text.length === 0) {
      return {
