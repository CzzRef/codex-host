...
export function modernSessionCapabilities(
  ...
  return {
    ...
    // line rather than being refused; DSH reads the file with its own tools.
...
export class ModernHarnessSession implements HarnessSession, ModernEventSink {
  ...
  /**
   * DSH names its own Sessions. `source.kind === "user"` means a person set the
   * title natively, which pins it against Host renaming.
   */
  #applyNativeTitle(data: Record<string, unknown>): void {
  ...
  #publishConfiguration(force: boolean): void {
    const next: HarnessSessionState = {
      ...this.#readConfiguration().state,
  ...
  /**
   * DSH injects a steer into the running native Turn with `mode: "steer"`, so
   * the Host Turn stays open instead of queueing a follow-up. Attachments
   * degrade to their path line for the same reason the start path does: the
   * prompt content this protocol accepts is text only.
   */
  async #steer(command: TurnSteerCommand): Promise<HarnessResult<TurnSteerAccepted>> {
    ...
    if (!active || active.turnId !== command.turnId) {
      return {
        ...
        error: {
          code: "invalidState",
          message: "DeepSeek Harness steer requires the active Turn",
          retryable: false,
        },
      };
    }
    ...
    if (
      ...
      command.input.some(
        (part) =>
          ...
          (part.type !== "file" || typeof part.path !== "string"),
      ) ||
      (hostInputText(command.input).length === 0 && hostInputFiles(command.input).length === 0)
    ) {
      return {
        ok: false,
        error: {
        ...
        this.#remote.call<unknown>(
          "session/prompt",
          {
            request: {
              ...
              sessionId: this.#sessionId,
              mode: "queue",
              content: promptContent(command.input),
            },
          },
          abort.signal,
        ),
  ...
  #projectLiveEvent(
    ...
    if (event.type === "session/title") {
      this.#applyNativeTitle(event.data);
      return;
    }
    if (!active) return;
...
/**
 * DSH prompt content is text-only, so each Host text part stays its own wire
 * part and every file part is appended as its deterministic path line rather
 * than being dropped.
 */
function promptContent(input: readonly HostInput[]): Array<{ type: "text"; text: string }> {
  const parts = input
    .filter((part): part is HostTextInput => part.type === "text")
    .map((part) => ({ type: "text" as const, text: part.text }));
  for (const file of hostInputFiles(input)) {
    parts.push({ type: "text" as const, text: hostFileInputLine(file) });
  }
  return parts;
}
