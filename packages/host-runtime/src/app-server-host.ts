...
export class AppServerHost {
  ...
  async #startExternalTurn(
    ...
    if (
      ...
      // A Host-owned direction change is its own stop-then-start lifecycle;
      // native steer injection must not race it, so only a Thread without a
      // pending steering takes the injection path.
      if (
        thread.activeTurnId &&
    ...
    if (this.#externalSteering.hasPending(thread.id)) {
      await replyError(-32072, "External Thread is changing direction");
      return;
    }
    try {
      ...
      try {
        await writer.json(rpcEnvelope(request, { result: { turn: started.turn } }));
        for (const peer of alsoAnswer) {
          if (peer.id === null) continue;
          await this.#writer.json(rpcEnvelope(peer, { result: { turn: started.turn } }));
        }
      } finally {
