...
| --- | --- | --- | --- | --- |
| Transcript Turn header | `implemented-local / focused-verified / live-pending` | [spec](260903/1631-transcript-turn-header/spec.md), [raw](260903/1631-transcript-turn-header/raw-requirement.md), [changes](260903/1631-transcript-turn-header/changes.md), OpenSpec [add-composer-workspace-bar](../../openspec/changes/add-composer-workspace-bar/tasks.md) §13 | `npm run typecheck` pass; `npm run lint` (eslint + boundaries) pass; renderer vitest 35 files 244 pass; Playwright `renderer-composer-workspace-surface` 1 pass; `npm run build:renderer` pass; per commit | Replaces hover `⋯` chip, floating cluster and bottom bar; header top inset below `header[data-pip-obstacle="app-shell-header"]`, user-bubble marker and paginated `M` are `[待真机]`; Side Chat sharing one scroller mounts no header |
...
- 2026-09-03 18:1x (Transcript Turn header, worktree `codex-conversation-display-f54032`): `npm run typecheck` pass; `npm run lint` pass; renderer vitest 35 files 244 pass; Playwright `renderer-composer-workspace-surface` 1 pass; `npm run build:renderer` pass. Live checks wait for the relaunch (header top inset, user-bubble marker, `thread/reverted` re-read).
