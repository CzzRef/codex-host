...
- Modify（2026-09-04 第二轮回测）：无原生铅笔时「编辑」= 编辑重发——先回滚到本轮之前（`numTurns = laterTurns + 1`）再回填输入框，始终二次确认；第一轮与 `lastTurnOnly` 且后面还有轮次时退回「追加」并在 tooltip 说明。`scrollToTurn` 改为 `scrollIntoView` + 有界逐帧校正（paginated 线程上直接写 `scrollTop` 只走 ~130px）。箭头边界留在 transcript 窗口内（fork 线程会带一条 Desktop 从不渲染的继承首轮）。
...
| E15 | 09-04 11:5x–12:2x | 用户第二轮反馈：①「编辑」把提示回填到底部输入框，发送等于在最后追加一条，而不是「回滚 → 清理 → 重新开始」；② 点提示词想跳到第 3 轮却跳到第 1 轮。核验结论：①**是真缺陷**——`runRollback` 用 `numTurns = laterTurns`，保留被编辑的那一轮，于是重发变成重复提问；Host 侧 `executeCheckpointRollback` 只要求 `retainedCount >= 1`，所以「连本轮一起回滚」在第 2 轮及之后完全可行，已改为 `laterTurns + 1` 并始终二次确认，第一轮与 `lastTurnOnly` 退回追加。真机实测：Desktop 只给**最后一轮**渲染 `Edit message` 铅笔，所以外部线程上非最后一轮走的正是被修的回填路径。②**根因是滚动被截断**：paginated 线程上 `scrollTop += 928` 只走 ~130px（实测三次 126/182/928 不定），旧实现一次 `scrollBy` 就停在邻轮；连点两次就会 3→2→1，正好复现用户描述。改为 `scrollIntoView({block:"start"})` + 有界逐帧校正后，健康线程上 4/4→3/4→2/4→3/4→4/4 全部正确，连点三次提示词停在同一轮。附带发现：`b4c9547f` 这条 fork 线程 Host 有 4 轮而 Desktop 只渲染 3 轮（继承的首轮从不渲染），任何程序化跳转 1–2 s 后会被 Desktop 拉回顶部——属 Desktop 侧行为，据此把箭头边界留在 transcript 窗口内。另：用户自己在 `4b6d3ba3` 上点过「编辑」并确认，Host 已回滚到 1 轮而 legacy transcript 仍显示 5 轮，Redo 槽仍在 |

## Closeout

- 2026-09-04 第二轮判断（用户可推翻）：只有「编辑」改成含本轮回滚，「回滚」按钮维持「取消之后轮次、保留本轮」不变；官方铅笔路径完全不碰（Desktop 自己就是编辑重发）。用户要求的「清理对话记录」在 paginated 线程上由 Host 补发 `thread/reverted` 自动完成，legacy 老线程仍只有提示——要让 legacy 也刷新需要改 Host 的 `#notifyExternalHistoryReplaced` 并真机验证，未做。
