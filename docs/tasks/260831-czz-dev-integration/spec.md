...
日期：2026-08-31；状态：源码安装及限定核验完成；Desktop 激活由用户暂缓。决定来源：explicit-current-request。
...
- 不提交、不推送，不改变 GitHub 仓库设置；远端默认分支与本地当前开发分支分开记录。
...
- 编译、协议握手、模型目录、原生会话和 Desktop UI 分层验收；当前桌面接入因用户的重启约束保留为待执行。
