...
- **Pin**：此前外部会话的 `thread/metadata/update`（isPinned）一律回 `-32078`，且外部行被排除在「已固定」列表之外。现在 pin 状态持久化到 mapping-store（`pinned` 字段），投影与 `thread/list` 的 `isPinned` 过滤按持久化状态生效；应答与官方 Codex 一样返回 `{ thread }`（含 `isPinned`），避免侧栏乐观置顶后因空结果闪回原位。gitInfo 等其余元数据仍显式关闭。契约：[external-thread-list-archive-routing](../openspec/specs/external-thread-list-archive-routing/spec.md)。
