...
这些修复都在 Host/Adapter 层，需要下一次正常退出后经 `codexhost launch` 重启 Desktop 生效：

- **Pin**：当前 Desktop 侧栏置顶走 `thread/section/move`（迁入内置 pinned section `01984de2-8f74-7c91-a3b2-5c5e937cf318`），再用 `thread/list` 的 `sectionId` + `sortKey=section_position` 拉已固定列表。此前 Host 对这条 `thread/*` 回 `-32076`，乐观置顶会在一秒内弹回。现在 External 的 section 成员写入 mapping-store，`thread/section/move` 成功回 `{}`，列表/read 投影 `section.id`；`thread/metadata/update` 的 `isPinned` 仍作为同一成员关系的兼容写入。gitInfo 等其余元数据仍显式关闭。契约：[external-thread-list-archive-routing](../openspec/specs/external-thread-list-archive-routing/spec.md)。
- **标题同步**：外部线程的名字此前只在创建时由 Desktop 兜底写入（首条消息截断），原生会话改名后不同步。现在 Adapter 把原生标题放进 `session.state.changed`（`HarnessSessionState.nativeTitle`），Host 持久化（`titleSource: native`）并发 `thread/name/updated`。来源：Claude Code 用 SDK sessionInfo 的 `customTitle ?? summary`（每轮结束轮询）；Grok 读 `summary.json` 的 `session_summary` + `title_is_manual`（每轮结束）；OMP 取会话文件最后一条 `type:"title"` 记录（轮次落盘识别时）；DSH 处理 `session/title` 事件（实时推送）。Pi 额外进程走委派 CLI `codexhost thread rename`（`/v1/thread/rename`），同一条 `thread/name/updated` 通知 Desktop；Cursor 仍无原生标题。归属规则：原生 user 改名总是覆盖；generated 标题只覆盖空名、原生已接管的名字或首条消息兜底名，不覆盖用户在 Desktop 里改出的自定义名。委派 CLI 改名把无可用 preview 的 `titleSource=desktop` 视为首条消息兜底而非手改名，允许覆盖；有 preview 证据且与首条消息不同的 Desktop 手改名仍拒绝覆盖。契约：[versioned-renderer-agent-routing](../openspec/specs/versioned-renderer-agent-routing/spec.md)、[add-external-thread-rename](../openspec/changes/add-external-thread-rename/specs/cross-harness-delegation/spec.md)。
- **排队消息**：Desktop 会把 `turn/start` 的任何拒绝标成永久 paused 的红色芯片；运行中点 Retry/Steer / 强制追加实际发 `turn/steer`。活动轮次内的 `turn/start` 由 Host 持有（上限 8 条）；当前轮**成功结束**后把队列里连续的 follow-up **合并成一条**后续 Turn（按到达顺序拼接），每条被持有的 RPC 都返回这一条，避免 Desktop 标成永久 paused。`turn/steer` 立即空结果应答（composer 可连续追加）：Session 声明 `turns.steer` 时注入**当前 Turn**（Claude PushableInput；Grok 同一 Host Turn 内立即续跑下一条 Prompt）；不支持原生 steer 时才入队，并在立即追加时取消当前 Turn 以便按序立刻启动。空闲则立即启动新 Turn。用户 Stop（`turn/interrupt`）仍丢弃队列；steer 触发的取消保留队列。其余非 start/interrupt/steer 的 `turn/*` 仍显式拒绝且不漏发官方 app-server。契约：[registered-harness-routing](../openspec/specs/registered-harness-routing/spec.md)、[add-external-turn-steer-queue](../openspec/changes/add-external-turn-steer-queue/specs/registered-harness-routing/spec.md)。
