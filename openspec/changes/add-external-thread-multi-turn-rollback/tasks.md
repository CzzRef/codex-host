...
- [ ] 3.6 `[live]` Confirm Desktop's paginated transcript re-reads on `thread/reverted` after a Renderer-initiated rollback (legacy-mode confirmed 2026-09-03 on a CLI-started Pi Thread: Host rolled back and restored 22 Turns, Desktop kept its 15-Turn window; the Turn header now says so)
- [x] 3.7 Inspect publishes ordered `turnIds` so the Turn header counts a rollback from Host truth instead of Desktop's virtualised transcript window (live 2026-09-03: a 22-Turn Thread showed 15 nodes at the top and a DOM count would have kept 9 Turns instead of 2)
