...
- [x] 13.3 Header row 2: workspace chips with `+N` collapse / click-expand / hover-preview, disclosure opening downward, preview beside the list between header and Composer; bottom bar and its `padding-bottom` reservation removed; native Changes / Review hiding condition unchanged
- [x] 13.4 Composer E2E for row 2: collapse / expand at a narrow width, downward list, preview placement, native controls hidden, no bottom bar, transcript end reachable above the Composer
- [x] 13.5 Preview page, proposal / design, task doc (`vibe/specs/260903/1631-transcript-turn-header/`), hub, README matrix (zh / en / ko), update-impact inventory, glossary
- [x] 13.6 `[live]` 2026-09-03 20:33 source relaunch (Desktop 26.831, PID 15137): header at `y=46/47` under the 46px `app-shell-header`, `x=846 w=736` = Composer, opaque, rect identical across four scroll positions; column-reverse scroller (`scrollTop` −656 at top, 0 at bottom) with its own 78px / 32px padding, reservation 49px so the first Turn starts under the header; prompt pinned only after the `[data-user-message-bubble]` bubble scrolls out; `Turn 1/5 → 2/5 → 4/5 → 5/5` while scrolling a 5-Turn Thread; native pencil present on official Turns; rollback + Redo on a scratch Pi Thread succeed at the Host. Found and fixed: `history-gap:` placeholder counted as a Turn, bubble-less Turns pinned assistant text, DOM-window rollback count (now `turnIds`), no way to target an earlier Turn on a short transcript (now arrows). Still open: paginated `thread/reverted` re-read (only a legacy Thread was available), Stop-label while streaming, native edit mode DOM
- [~] 13.7 `[live]` Relaunch with 891e4fe / 18b4a46 / the `turnIds` fix and re-check the gap placeholder, arrows, and a rollback from the header against `codexhost thread read` — 2026-09-04: Desktop auto-updated to 26.901 (inspect fuse off); upstream v0.4.4 CDP injection merged as `4b45876` and the main checkout rebuilt; 2026-09-04 12:1x on 26.901.22334: gap placeholder excluded (`Turn 1/6` with 1 gap in 7 nodes), arrows step, header pinned at y=46, external Thread opens (`2d8a381`); rollback-count confirmation on the 22-Turn scratch Thread still unread because the user was driving Desktop at the same time
- [x] 13.8 `[live]` 2026-09-04 11:0x–11:4x on 26.901.22334 (Desktop launched by `codexhost launch` 10:58, CDP 62118, rebuilt Renderer hot-reinstalled through `installRendererBinding`'s own dispose): audited the shipped header against the raw requirement and fixed five defects — duplicate prompt while the bubble sits in the transparent 0–46px chrome band (`promptPinned` now bounded by the scroller top), current-Turn probe line below the header so the prompt button and `›` no longer resolve back to the previous Turn, second row removed when the conversation has no changed files (header 73px → 41px) with the core chip riding in the Turn row and yielding to a pinned prompt, `Turn N/M` taken from the Host's `turnIds` so a virtualised window no longer labels a 5-Turn Thread `Turn 1/3`, expand chevron only when the line actually clipped and the panel flush under the header. Verified live: rect stable at `x=846 y=46 w=736 h=41` across eight scroll positions, prompt hidden while the bubble is readable at `bubbleBottom` 11/29/48, `Turn 5/5` with 4 DOM Turns and `Turn 1/5` with 3, prompt click and both arrows keep their Turn after the 600ms override expires, first Turn clears the header at scroll-top (95 ≥ 87), single-Turn Thread hides the arrows
- [x] 13.9 `[live]` 2026-09-04 11:5x–12:2x, second audit round against the raw requirement: Edit now means "edit and resend" — without a native pencil it rolls the Thread back past the current Turn (`numTurns = laterTurns + 1`) so the resend replaces it instead of appending a duplicate, always behind a confirmation, falling back to append-only on the first Turn (the Host keeps it) and on a `lastTurnOnly` Harness with Turns after the current one. `scrollToTurn` now uses `scrollIntoView` plus a bounded per-frame correction: measured on a paginated Thread a direct `scrollTop` jump of 928px lands only ~130px, which is what made the prompt button and `›` stop on a neighbouring Turn. Step arrows stay bounded by the transcript window (a forked Thread carries an inherited first Turn Desktop never renders, so Host-bounded arrows would be dead). Verified live: Edit copy resolves `native` on the last Turn (Desktop renders its pencil there only), `replace` elsewhere, `append` on the first Turn; stepping 4/4 → 3/4 → 2/4 → 3/4 → 4/4 on a healthy Thread; three repeated prompt clicks stay on the same Turn. Known Desktop-side limit: on one forked Thread whose first Host Turn is never rendered, Desktop drags the transcript back to the top ~1–2 s after any programmatic jump.
- [x] 13.10 2026-09-05 follow-ups from live use: workspace chips carry their own hover tooltip instead of a native `title` (the OS delay read as the chip being slow); a picked Host-managed worktree survives Desktop's run-location drifting back to `worktree` (it used to be discarded, so the Thread started in the project root); `New worktree…` prefills a complete `yyMMdd-core` name derived from the draft prompt (`suggestWorkspaceWorktreeName`), so no naming step is needed on any Harness. Gates: typecheck / lint / vitest 210 files 1865 pass / Playwright composer spec pass


## 14. Approved workspace and turn interaction redesign (2026-09-08)

The earlier checked slices are historical delivery records. This slice supersedes their layout/hover behavior and the append-only Edit fallback; the current implementation and validation owner is [260908 workspace redesign](../../../vibe/specs/260908/0905-workspace-interaction/spec.md).

- [x] 14.1 Separate the Composer workspace dock from the pinned turn header; preserve core identity with no file changes and release layout reservations on unmount
- [x] 14.2 Loaded-turn menu, stable-key jumps and local task-clause subtitles; use the complete prompt for edits and confirmations
- [x] 14.3 Explicit, persistent file detail with resize, native-open, Escape, focus restoration and Composer/Thread ownership
- [x] 14.4 Worktree full paths, current send directory, refresh, creation destination hint and safe fallback entry; honor rejected policy selections; reuse the menu from current conversations, navigating through a verified native new-conversation control before applying a pending cwd
- [x] 14.5 Disable edits that cannot replace the selected turn; retain native edit and existing rollback/Redo contracts
- [x] 14.6 Final offline checks and documentation closeout: 34 focused unit tests, production-component E2E, typecheck, Renderer build, lint/boundaries, formatting, diff and local-link checks; exact scope recorded by the implementation owner
- [ ] 14.7 Real Desktop routing, rendering and interaction acceptance; integration and installed runtime update
