...
- **风险：`path-text` 的文本注入**（切片 1/2 实现后修正了原先的表述）。降级文本会拼进用户提示词。原本写成「用固定格式使用户输入无法伪造」，这是**过度承诺**：codexhost 只生成这一行、从不把它解析回文件部件，所以这里根本没有可被骗的解析器；用户手打一行相似文本去误导 Agent，是任何文本通道都固有的，格式设计防不住。能真正保证的只有一条——路径本身不得引入额外行。因此实现对路径中的 `\r\n` 做折叠，并加了针对性测试。
