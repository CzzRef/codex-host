...
/// (`<root>/<target-dir>/{debug,release}/codexhost`) instead of an install tree.
///
...
///
/// Unlike [`InstalledResources::from_source_checkout`], which resolves the
/// whole resource set for the canonical `target/` directory without touching
/// the file system, this helper is the existence-checked fallback: it accepts
/// any Cargo target directory name and only answers when the built bundle is
/// actually present.
